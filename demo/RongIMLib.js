...
(function (RongIMLib) {
    var RongIMClient = (function () {
        ...
        RongIMClient.prototype.getSendingBox = function (callback) {
            RongIMLib.CheckParam.getInstance().check(["object"], "getSendingBox");
            var modules = new Modules.SyncRequestMsg(), self = this;
            modules.setIspolling(false);
            modules.setIsweb(true);
            modules.setSyncTime(RongIMClient._memoryStore.lastReadTime.get("sendbox" + RongIMLib.Bridge._client.userId));
            RongIMClient.bridge.pullSendBoxMsg("pullMsg", RongIMLib.MessageUtil.ArrayForm(modules.toArrayBuffer()), RongIMLib.Bridge._client.userId, {
                onSuccess: function (data) {
                    RongIMClient._memoryStore.lastReadTime.set("sendbox" + RongIMLib.Bridge._client.userId, RongIMLib.MessageUtil.int64ToTimestamp(data.syncTime));
                    var list = data.list.reverse();
                    for (var i = 0, len = list.length; i < len; i++) {
                        list[i] = RongIMLib.MessageUtil.messageParser(list[i]);
                    }
        ...
        RongIMClient.prototype.getQnTkn = function (fileType, callback) {
...
(function (RongIMLib) {
    ...
    var Bridge = (function () {
        ...
        Bridge.prototype.pullSendBoxMsg = function (topic, content, targetId, callback, pbname) {
    ...
    var FeatureDectector = (function () {
        function FeatureDectector() {
            ...
            if ("WebSocket" in window && "ArrayBuffer" in window && WebSocket.prototype.CLOSED === 3 && !window["WEB_XHR_POLLING"]) {
                ...
                this.script.src = RongIMLib.MessageUtil.schemeArrs[RongIMLib.RongIMClient.schemeType][0] + "://cdn.ronghub.com/protobuf-min-2.6.js";
